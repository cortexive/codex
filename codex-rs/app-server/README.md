...
- `thread/name/set` — set or update a thread’s user-facing name for either a loaded thread or a persisted rollout; optional `workflowDisplay` carries ephemeral workflow context to initialized clients without persisting it. Returns `{}` on success and emits `thread/name/updated`. Thread names are not required to be unique; name lookups resolve to the most recently updated thread.
